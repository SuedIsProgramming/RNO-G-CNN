...
event_dict = {} # Event dictionary will contain n dictionaries for each event with {'mean_integrated_power','data'} key pairs.
...
for iEvent, event in enumerate(events): # For each event
    ...
    for iStation, station in enumerate(event.get_stations()): # For each station
        ...
        for iChannel, channel in enumerate(station.iter_channels()):
            ...
            intpow_mean += channel.get_parameter(param.SNR)['integrated_power'] # Add integrated powers for each channel
        ...
        event_dict[iEvent] = {'mean_integrated_power' : intpow_mean, 'data' : np.array(e_data)} # Populate event dictionary
